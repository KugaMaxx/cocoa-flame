...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask=None, pos_embed=None, query_embed=None):
        batch_size, num_samples, feat_dims = src.shape

        # flatten NxCxD to DxNxC
        src = src.transpose(1, 2).permute(2, 0, 1)
        pos_embed = pos_embed.transpose(1, 2).permute(2, 0, 1)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False):
        super().__init__()
        # # TODO try 1: Self Attention
        # self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)

        # TODO try 2: External Attention
        self.ea_attn = ExternalAttention(d_model)
